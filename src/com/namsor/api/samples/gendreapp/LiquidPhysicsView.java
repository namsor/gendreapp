 ...
 * Not working :-(
 */
public class LiquidPhysicsView extends View {

	public LiquidPhysicsView(Context context) {
		super(context);
		init();
	}

	public LiquidPhysicsView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	public void init() {
		...
		// createParticles(GenderStats.getFemaleCount(),
		// GenderStats.getMaleCount());
		updateParticles();
	...
	private final Runnable mTimerRunnable = new Runnable() {
		...
		public void run() {
			...
			mFrameRate = new String("GendRE" + " ♂:"
					+ GenderStats.getMaleCount() + " ♀:"
					+ GenderStats.getFemaleCount());
...
//		final SurfaceHolder holder = getSurfaceHolder();
		...
		for (Particle p : getParticles()) {
			Paint paint = (p.polarityPlus ? linePaintPlus : linePaintMinus);
			c.drawCircle((float) (8.0F * p.x), (float) (8.0F * p.y), RADIUS,
					paint);
			if (!p.polarityPlus) {
	...
	public synchronized void createParticles(int newCountPositive,
		...
		int nN = (int) Math.round(10F * (1 - r)
				+ Math.log(newCountPositive + newCountNegative) * 50 * (1 - r));

		for (int j = 0; (cN < nN || cP < nP) && j < 50; j++) {
			for (int i = 0; cN < nN && i < 10; i++) {
				Particle p = new Particle(this.water, i + 4, j + 4, 0.0F, 0.0F,
						false);
				getParticles().add(p);
				cN++;
			}
	...
	public void simulate() {
		...
		for (Particle p : getParticles()) {
			...
			float v2 = v * v;
			float v3 = v * v2;
			float density = p00 + x00 * u + y00 * v + C20 * u2 + C02 * v2 + C30
